...
async function WatchSASS() {
  const controller = new Promise(() => {
    const command = spawn("sass", ["--source-map","sass:assets/css"]);

    command.stdout.on('data', function (data) {
      const output = chalk.rgb(144, 238, 144)(`${data.toString()}`);
      const titling = chalk.rgb(170, 152, 169)("stdout | SASS: ");
      console.log(`${titling}${output}`);
    });
    
    command.stderr.on('data', function (data) {
      const output = chalk.rgb(168, 58, 58)(`${data.toString()}`);
    ...
    command.on('exit', function (code) {
      ...
      let coloredCode = chalk.rgb(144, 238, 144)(code.toString())
      if (code !== 0) {
        coloredCode = chalk.rgb(168, 58, 58)(code.toString())
      }
